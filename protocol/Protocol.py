...
class Protocol:
	...
	def _checkCompat(self, client):
		...
		if not client.compat['sp']: # blocks protocol increase to 0.37
			...
			self._root.console_write('Handler %s:%s <%s> client "%s" missing compat flag: sp '%(client.handler.num, client.username, client.session_id, client.lobby_id))
	...
	def _isNoob(self, client):
		...
		if not client.access in ('mod', 'admin') and not client.bot and not (client.ingame_time > 0):
			self.out_SERVERMSG(client, "You are not allowed to do this, you need to play a bit more, please ask in the channel #main!", True)
			return True
		return False
